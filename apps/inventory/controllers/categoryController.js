...
exports.createIvmCategory = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ _id: savedCategory._id, name: savedCategory.name, items: [] });
  } catch (err) {
...
exports.updateIvmCategory = async (req, res) => {
  try {
    ...
    const items = await Item.find({ categoryId: updatedCategory._id }).populate(
      "tags"
    );
...
exports.addItemToIvmCategory = async (req, res) => {
  try {
    ...
    if (!category.restaurantId.equals(restaurantId)) {
      return res
        .status(403)
        .json({ message: "Not authorized to add items to this category" });
    }
    ...
    const newItem = new Item({
      ...
      quantity,
      avgPrice: price,
      totalPurchaseValue:price*quantity,
